...
exports.s4_GET = function (req, res, next) {
  try {
    ...
    if (!range) {
      ...
      const MAX_CHUNK_SIZE = 1000 * 1000; // 1MB
      ...
      const header = {
        ...
        // "Content-Length": fileSize - 1, // 파일의 전체 길이를 클라이언트에게 알려준다. - ERR_CONTENT_LENGTH_MISMATCH - WHY?????
      };
...
exports.s5_GET = function (req, res, next) {
  try {
    const filename = req.query.file;
    const filepath = `./resource/${filename}`;
...
exports.s5_upload = function (req, res) {
  const body = `
    <html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
    ...
    <body>
        <form action="/s5" enctype="multipart/form-data" method="post">
            <input type="file" name="file1" multiple="multiple">
            <input type="submit" value="Upload file" />
        </form>
    </body>
...
exports.s5_POST = function (req, res, next) {
  try {
    const form = new multiparty.Form();
    form.on("error", (err) => res.status(500).end());
    form.on("part", (part) => {
